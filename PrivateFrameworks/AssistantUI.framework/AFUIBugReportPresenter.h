/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/AssistantUI.framework/AssistantUI
 */

@interface AFUIBugReportPresenter : NSObject <AFUIBugReportPresenting, AFUIDebugControllerDelegate, AFUIInternalDebugControllerDelegate> {
    <AFUIDebugControlling> * _debugController;
    <AFUIBugReportPresentingDelegate> * _delegate;
    UIViewController<AFUIInternalDebugControlling> * _internalDebugController;
    UIAlertController * _presentedAlertController;
    UIViewController * _presentedTTRController;
}

@property <AFUIBugReportPresentingDelegate> *delegate;
@property (setter=_setPresentedAlertController:, nonatomic, retain) UIAlertController *presentedAlertController;
@property (setter=_setPresentedTTRController:, nonatomic, retain) UIViewController *presentedTTRController;

- (void).cxx_destruct;
- (void)_actionClickedForAction:(id)arg1 withActionType:(long long)arg2;
- (id)_actionForAlertAction:(long long)arg1;
- (long long)_actionStyleForAlertAction:(long long)arg1;
- (id)_actionTitleForAlertAction:(long long)arg1;
- (id)_alertControllerWithActionTypes:(id)arg1;
- (void)_captureViewHierarchyDescriptionForController:(id)arg1;
- (id)_confirmationAlertControllerWithTitle:(id)arg1 onConfirmation:(id /* block */)arg2;
- (id)_conversationDataFromConversation:(id)arg1;
- (void)_debugControllerPresentationDidEnd;
- (void)_debugControllerPresentationWillStart;
- (void)_dismissInternalDebugController;
- (void)_dismissViewControllerAnimated:(bool)arg1;
- (double)_logTime;
- (void)_requestDeviceUnlockWithSuccessCompletion:(id /* block */)arg1 withErrorCompletion:(id /* block */)arg2;
- (void)_requestToOpenURL:(id)arg1;
- (void)_requestToPresentAlertController:(id)arg1;
- (void)_requestToPresentDebugController:(id)arg1;
- (void)_requestToPresentInternalDebugController:(id)arg1;
- (void)_requestToPresentViewController:(id)arg1;
- (void)_setPresentedAlertController:(id)arg1;
- (void)_setPresentedTTRController:(id)arg1;
- (id)_ttrController;
- (id)_utterancesFromConversation:(id)arg1;
- (void)debugController:(id)arg1 openURL:(id)arg2;
- (void)debugController:(id)arg1 requestsPermissionToScreenshot:(id /* block */)arg2;
- (void)debugController:(id)arg1 requestsScreenShotWithCompletion:(id /* block */)arg2;
- (void)debugController:(id)arg1 stateDidChangeWithAddViews:(id)arg2;
- (void)debugController:(id)arg1 utterances:(id)arg2 openTTR:(id /* block */)arg3;
- (id)debugControllerRequestsBugReportKeywordIdentifiers:(id)arg1;
- (id)debugControllerRequestsBugReportPrefixForTitle:(id)arg1;
- (id)delegate;
- (void)didReceiveBugButtonLongPress;
- (void)didReceiveReportBugAction;
- (void)dismiss;
- (bool)hasContentAtPoint:(struct CGPoint { double x1; double x2; })arg1;
- (id)initWithDelegate:(id)arg1;
- (void)internalDebugControllerWantsDismissViewController;
- (id)presentedAlertController;
- (id)presentedTTRController;
- (void)setDelegate:(id)arg1;

@end
