/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/SessionUIServices.framework/SessionUIServices
 */

@interface SessionUIServices.ActivityItemMetricsRequest : NSObject <BSXPCCoding, NSCopying, NSSecureCoding> {
    void cornerRadius;
    void heightRequest;
    void widthRequest;
}

@property (nonatomic) double cornerRadius;
@property (nonatomic, retain) _TtC17SessionUIServices20SizeDimensionRequest *heightRequest;
@property (nonatomic, readonly) struct CGSize { double x1; double x2; } initialSize;
@property (nonatomic, readonly) struct CGSize { double x1; double x2; } maximumSize;
@property (nonatomic, retain) _TtC17SessionUIServices20SizeDimensionRequest *widthRequest;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)copyWithZone:(void*)arg1;
- (double)cornerRadius;
- (void)encodeWithCoder:(id)arg1;
- (void)encodeWithXPCDictionary:(id)arg1;
- (id)heightRequest;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (id)initWithWidth:(id)arg1 height:(id)arg2 cornerRadius:(double)arg3;
- (id)initWithXPCDictionary:(id)arg1;
- (struct CGSize { double x1; double x2; })initialSize;
- (bool)isValid;
- (struct CGSize { double x1; double x2; })maximumSize;
- (void)setCornerRadius:(double)arg1;
- (void)setHeightRequest:(id)arg1;
- (void)setWidthRequest:(id)arg1;
- (id)widthRequest;

@end
